...
reg [4:0] j;
reg [4:0] temp;
parameter [6:0] infinity = 7'b1111111;
reg [4:0] cost_matrix[0:25][0:25];
reg [12:0] dist_h [0:25];
reg [12:0] dist [0:25];
reg [10*5-1:0] final_path_reg = {{5'd27} * 10};
parameter IDLE = 0, S0 = 1, S1 = 2, S2 = 3;

initial begin

	for(i = 0; i<node_count-1;i=i+1) begin
		dist[i] = {1'b0,s_node[4],s_node[3],s_node[2],s_node[1],s_node[0],{7'b1111111}};
		dist_h[i] = {1'b0,s_node[4:0],{7'b1111111}};
	end
...
reg [12:0] min = 0;
reg [4:0] min_index = 0;
reg [12:0] sel = {1'b1,{1'b0,1'b0,1'b0,1'b0,1'b0},{7'b1111111}};
	...
	case(next_state)
		...
		S0: begin //cost_matrix se value liya, we'll check if it's less than the near_row_hold values and if yes, we'll change the near_row array.
			...
			if(visited_count == 24) begin
				next_state <= S1;
				j=e_node;
				dist_h[0][11:7] = 5'b11111;
				$display("DONE");	
			end
			else if(i == node_count-2) begin
				min <= {1'b1,sel[11:0]};
				visited_count <= visited_count + 1;
				dist[min_index][12] = 1'b1;
				...
				j = min_index;
				i = -1;
				sel =  {1'b1,{min_index[4:0]},{7'b1111111}};
			end
			else if ( ((cost_matrix[j][i] + min[6:0]) < dist_h[i][6:0]) && (dist_h[i][12] != 1) ) begin
				dist[i] = {1'b0,j,{cost_matrix[j][i] + min[6:0]}};
